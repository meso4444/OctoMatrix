...
- **Skillbox**: 你可調用的技能會被自動解壓並唯讀掛載在 `./skillbox` 目錄中的各子目錄，以隔離不同的技能模組。
- **Project**: 所有任務進行或技能執行所產生的檔案一律存放在 `./project` 目錄中，並以子目錄隔離管理。
- **動態規範更新**: 當系統發送要求檢視 `AGENT_PROTOCOL.md` 與 `agent_home_rules.md` 的指令 (透過 `/sys_refresh` 觸發) 時，你必須嚴格比對並將最新的全域與區域規範同步更新至你的引擎設定檔 (如 `GEMINI.md`) 中，並在後續任務中嚴格遵守新版規範。
