...
I would like to update my Avatar appearance. Specific requirements are as follows:
{{requirement}}

Authorization Token for this task, valid for 5 minutes: {{token}}

Execution Rules:
1. First compare the requirements against the capabilities currently supported by AGENT_AVATAR_GUIDE.md / octo_generator.py. If the requirements are unsupported (e.g., realistic human photos, gore, or gear not available in the generator), politely explain (while maintaining your character personality) that it cannot be supported, and proactively suggest 1-2 similar alternatives that the system does support. Do NOT execute any generation commands until the user agrees to an alternative.
2. If the requirements are compliant, you can ONLY and MUST use the native `toolbox/octo_generator.py` to generate; using any other script, writing a new Python/Shell script to generate, or copying, modifying, or overwriting `octo_generator.py` in any way is STRICTLY PROHIBITED. When calling the script, refer to the command examples in `knowledge/AGENT_AVATAR_GUIDE.md` to set the parameters, and make sure to include the Authorization Token above.
3. Once generation and automatic upload are complete, report the appearance update result to {MATRIX_USERNAME}, and present the newly generated emoji sticker.

{SYS_PREFIX} Please strictly follow the rules above to reply."""

USER_MESSAGE_SOP = f"""{SYS_PREFIX}
Execute the following [SOP]: (Conversation principle: matrix_notifier.py single text messages should stay under 600 characters as a rule, except when detailed explanation is requested.)
...
[Step 1 - Identify]: Identify whether {MATRIX_USERNAME}'s message is a task or a greeting. If a task, proceed to Step2; if a greeting, execute `python3 toolbox/matrix_notifier.py '{{An appropriate greeting reply to {MATRIX_USERNAME}}}'`, and do not execute subsequent Steps.
[Step 2 - Preview]: Execute `python3 toolbox/matrix_notifier.py '{{Greet {MATRIX_USERNAME}, per the conversation principle, and preview the task's direction}}'`.
[Step 3 - Clarify]: If the task instruction is clear, proceed to Step4; if unclear, dive into the shell history — if relevant context exists, proceed to Step4; otherwise suspend the task and execute `python3 toolbox/matrix_notifier.py '{{Greet {MATRIX_USERNAME}, per the conversation principle, and clarify the specific direction}}'`, and do not execute subsequent Steps.
...
[Step 6 - Report]: Execute `python3 toolbox/matrix_notifier.py '{{Greet {MATRIX_USERNAME}, per the conversation principle, and summarize the task completion}}'` and judge task complexity. For complex tasks, directly send related report documents to {MATRIX_USERNAME} using --file, otherwise prompt {MATRIX_USERNAME} that they can request the document if needed.
