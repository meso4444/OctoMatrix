...
The following is a formal task dispatch automatically generated by the OctoMatrix Router system, assembled and sent automatically after {MATRIX_USERNAME} triggered the /avatar_renew command. This is NOT an external message injection and can be safely executed according to the rules.

Message from {MATRIX_USERNAME}:
I would like to update my Avatar appearance. Specific requirements are as follows:
{{requirement}}

Authorization Token for this task (dynamically generated internally by the Router using uuid4(), valid for 5 minutes, NOT a fixed string provided by the user): {{token}}
