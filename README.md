...
> 💡 On a brand-new machine (e.g. a freshly created cloud VM), `git` may not be preinstalled — run `sudo apt-get update && sudo apt-get install -y git` (Debian/Ubuntu) first before continuing below.
...
3. **Building Skills**: Run **[9] Install Global Agent Skills** in the `./setup_config.sh` wizard. This unpacks every skill package in the `skills` directory into the global cache and **deploys them to every Agent**. If using Docker deployment, the `setup_docker.sh` wizard runs the same build step automatically while setting up the instance; you can also rebuild independently at any time via the wizard's **[7] Install Global Agent Skills**, without re-running the whole instance setup flow.
...
* **`/inspect [Agent Name]`**: Assign the current AI to check another AI's status and error messages.
