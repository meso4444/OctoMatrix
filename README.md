...
3. **建置技能**：透過 `./setup_config.sh` 設定精靈的 **[9] 執行全域技能建置**。此操作會將 `skills` 目錄下所有技能包解壓至全域快取並**部署給所有 Agent**。若使用 Docker 部署，`setup_docker.sh` 設定精靈在設置實例時會自動執行相同的建置流程，將技能部署給所有 Agent。
