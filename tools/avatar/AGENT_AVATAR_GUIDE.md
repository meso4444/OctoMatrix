...
另外 `--item_r` / `--item_l` 皆可設為 `none`（不持道具，預設值），未計入以上 76 款計數。`chinese_lantern`(紅色中式燈籠)跟`candle_lantern`(西式蠟燭提燈，sumikko提供)是兩款視覺風格不同的獨立道具，選用時請注意風格區分。
