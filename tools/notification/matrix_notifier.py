...
Cross-platform Notification Center - Template translation and automatic failover

Responsibilities:
1. Unified interface: notify(target_platform, template_id, context)
2. Cross-platform template rendering: Compatible with old (icon/title/content) and new (platform-specific) templates
3. Automatic failover: If main channel fails, automatically switch to backup channel
...
try:
    ...
    # Prevent multi-user permission conflicts in container
    pass
...
class TemplateManager:
    ...
    def _load_templates(self, path: str) -> dict:
        ...
        try:
            ...
            logger.error(f"[Notifier] Template loading failed: {e}")
            return {}
    ...
    def render(self, platform: str, template_id: str, context: Dict[str, Any]) -> str:
        ...
        # 3. Get platform-specific content or execute Legacy merge
        template = template_entry.get(platform)
        if not template:
            # Process old icon + title + content structure
            ...
            if icon or title or body:
                ...
                template = f"[{template_id}] Notice: {context.get('content', '')}"
        ...
        if isinstance(template, str):
            try:
                ...
                logger.error(f"[Notifier] Rendering exception: {e}")

        # --- Cross-platform tag automatic translation (HTML -> Markdown) ---
        if platform == 'discord':
...
class TelegramSender:
    ...
    def send(self, chat_id: str, message: str, parse_mode: str = 'HTML', **kwargs) -> bool:
        try:
            ...
            for chunk in chunks:
                ...
                # Telegram exclusive keyboard support
                ...
                if resp.status_code != 200:
                    if parse_mode:
                        logger.warning(f"[Notifier] Telegram parsing error ({resp.status_code}), attempting to resend in plain text mode. Error: {resp.text}")
                        if 'parse_mode' in data:
                            del data['parse_mode']
                        resp = requests.post(f"{self.api_url}/sendMessage", json=data, timeout=5)
                        if resp.status_code != 200:
                            logger.error(f"[Notifier] Telegram plain text resend failed: {resp.text}")
                            success = False
                    else:
    ...
    def send_file(self, chat_id: str, file_path: str, file_type: str = 'document', caption: str = '', **kwargs) -> bool:
        ...
        try:
            # Sticker special processing: auto convert to WebP and force no caption
            if file_type == 'sticker':
                if file_path.lower().endswith('.webm'):
                    pass  # WebM dynamic stickers are natively supported, no conversion needed
                    ...
                    try:
                        ...
                        logger.warning(f"[Notifier] Sticker conversion failed: {e}. Falling back to original.")
                        target_path = file_path  # Restore path if conversion fails
                caption = "" # Stickers force no text
            ...
            # Auto-detect photo and if gif, change to sendAnimation
...
class SlackSender:
    ...
    def send(self, channel_id: str, message: str, **kwargs) -> bool:
        try:
            ...
            logger.error(f"[Notifier] Slack API exception: {e.response['error']}")
            return False
        except Exception as e:
            logger.error(f"[Notifier] Slack send unknown exception: {e}")
            return False
    ...
    def send_file(self, channel_id: str, file_path: str, file_type: str = 'document', caption: str = '', **kwargs) -> bool:
        ...
        try:
            ...
            logger.error(f"[Notifier] Slack file send exception: {e}")
            return False
        finally:
            if is_temp_webp and os.path.exists(target_path):
                try: os.remove(target_path)
...
class MatrixNotifier:
    ...
    def notify(self, target_platform: str, template_id: str, context: Dict[str, Any], target_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if not PLATFORMS_ENABLED.get(target_platform, True):
                logger.warning(f"[Notifier] Platform {target_platform} is disabled, skipping notification")
                return False
        except ImportError:
            pass
    ...
    def notify_file(self, target_platform: str, file_path: str, file_type: str = 'document', caption: str = '', target_id: Optional[str] = None) -> bool:
        ...
        try:
            from config import PLATFORMS_ENABLED
            if not PLATFORMS_ENABLED.get(target_platform, True):
                logger.warning(f"[Notifier] Platform {target_platform} is disabled, skipping file send")
                return False
        except ImportError:
            pass
...
def get_source_info():
    ...
    # Attempt to get default main channel and enabled status from config
    try:
    ...
    if args.file:
        ...
        try:
            with open(f_path, 'rb') as f:
                # [Optimization] Send only when target_id is valid to avoid passing invalid strings
