...
*   **參數說明**：
    ...
    *   `--level snapshot [--range START-END]`：讀取快照層級的聚合 GHOST，
        依新鮮度排序，1 為最新，**預設 `--range 1-30`**。找不到相關關鍵字
        可往回翻頁，例如 `--range 31-100`、`101-200`。輸出不做字母排序，
        建議每次分頁維持 30~100。
    *   `--level monthly --month YYYY-MM`：精準讀取指定月份的月度歸併索引
        （必填）。
    *   `--level yearly --year YYYY`：精準讀取指定年份的年度歸併索引（必填）。
    *   monthly／yearly 不支援主題篩選，輸出為字母排序，需自行判斷相關性。
    *   所有層級輸出的關鍵字會自動剝除開頭的 `-`（例如 `-C預設50改20` →
        `C預設50改20`），避免被 `dive_into_the_shell.py` 誤判為選項。
*   **執行範例**：
