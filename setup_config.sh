...
# ==============================================================================
# 防呆：非 TTY (無互動終端機) 時直接退出，避免主選單迴圈在無法讀取輸入的
# 環境下無限重印空轉 (例如 CI/自動化腳本誤呼叫、SSH 非互動連線)
# ==============================================================================
if [ ! -t 0 ]; then
    echo "❌ 偵測到非互動式終端機 (non-TTY)，setup_config.sh 需要互動輸入，已中止執行。"
    echo "   請在有終端機的環境下手動執行 ./setup_config.sh 完成設定。"
    exit 1
fi
