...
if [ ! -f "$SECRET_FILE" ]; then
    echo "❌ 錯誤: 找不到 Webhook Secret，請先執行 start_octo_services.sh"
    exit 1
fi
...
echo "🚀 正在啟動 ngrok (Port $TELEGRAM_GATEWAY_PORT)..."
...
if [[ -n "$NGROK_CONFIG_PATH" && -f "$NGROK_CONFIG_PATH" ]]; then
    ...
    echo "📋 使用默認配置 (Telegram Gateway Port $TELEGRAM_GATEWAY_PORT)"
